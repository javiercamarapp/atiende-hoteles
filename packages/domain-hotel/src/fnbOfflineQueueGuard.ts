 ...
 * (`fnb_offline_charge_queue`, tabla append-only) y aplica el resultado, reutilizando
 * `createRoomServiceCharge`/`reverseCharge` de `routes/folios.ts` -- este módulo es la
 * validación de dominio PURA (sin I/O) de la forma de un ítem de la cola antes de
 * intentar aplicarlo, mismo principio que `fnbAllergyGuard.ts`.
 *
 ...
 *  que sí tiene I/O) ni la identidad del huésped (REQ-AB-012, `assertRoomChargeIdentityVerified`
 *  en `folioEngine.ts`, invocada aparte por la ruta para operaciones 'cargo'). Esta
 *  función solo garantiza que el ítem esté completo y sea internamente coherente antes
