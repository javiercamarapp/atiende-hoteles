...
// recuperar señal. Reutiliza EXACTAMENTE `createRoomServiceCharge`/`reverseCharge` de
// `./folios.ts` -- nunca una segunda implementación de la verificación de identidad
// (REQ-AB-012) o del reverso que pudiera divergir.
//
...
import { loadFolio, createRoomServiceCharge, reverseCharge } from "./folios.ts";
...
const offlineQueueItemSchema = z.object({
  ...
  // REQ-AB-012: esta cola es exclusivamente de F&B (concept='ab' SIEMPRE) -- offline
  // no es una vía para saltarse la doble verificación de identidad, mismos campos que
  // `chargeSchema` en `./folios.ts`.
  verificacionIdentidad: identityClaimSchema.optional().nullable(),
      ...
      async () => {
        ...
        try {
          ...
          if (item.operationType === "reverso") {
            ...
            // 'cargo': esta cola es exclusivamente de F&B -- siempre concept='ab',
            // por lo que SIEMPRE exige la doble verificación de identidad de
            // REQ-AB-012 (el mismo camino que la ruta normal de cargos, vía
            ...
            const created = await createRoomServiceCharge({
              ...
              autorizacionIdentidadPorUserId: item.autorizacionIdentidadPorUserId,
              auditPayload: { ...item, viaOfflineQueue: true },
            });
            resultChargeId = created.id;
          }
