...
// cuando el desplazamiento es distinto de cero). Rutas bajo
// `/hoteles/:hotelId/grupos/cotizaciones*`.
//
...
// obligatoria antes de enviar cualquier propuesta de RFP." Este mismo archivo es
// también la única superficie HTTP de `solicitud_grupo`/`seguimiento_solicitud_grupo`/
// `validacion_humana_rfp`/`propuesta_rfp` (packages/db/migrations/
// 0130_seguimiento_solicitud_grupo.sql). Rutas bajo `/hoteles/:hotelId/grupos` (bare)
// y `/hoteles/:hotelId/grupos/:id*` -- deliberadamente el mínimo real que hace posible
// el seguimiento y el gate de RFP, SIN solaparse con el room-block/cotización de
// REQ-RES-012 de arriba: Hono resuelve `/grupos/cotizaciones/:id` (2 segmentos fijos +
// param) y `/grupos/:id` (1 param) como patrones distintos, y los métodos HTTP de
// `/grupos`/`/grupos/cotizaciones` (bare, sin id) tampoco se pisan entre sí.
//
// Reconciliación de fusión (closure/todos-los-req-hoteles-lote1, 2026-09-11): las dos
// ramas crearon este archivo de forma independiente (conflicto add/add real, sin
// solape textual) -- se combinan aquí completas, cada bloque conservando su propia
// lógica intacta.
//
...
// --- REQ-RES-012: cotización / room-block -----------------------------------------
...
// --- REQ-RES-013: solicitud / seguimiento / RFP -------------------------------------
