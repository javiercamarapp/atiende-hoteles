...
//   - `reservation.confirmed` -> confirmación de reserva: el HANDLER está completo y
//     probado, pero `routes/reservas.ts` (fuera de alcance de este agente, lote B) NO
//     inserta todavía un evento `outbox` en la transición a 'confirmada' (solo emite
//     `reservation.created`, que ocurre con estado 'cotizada', no 'confirmada' --
//     ver domain/reservation, no se puede fabricar la semántica correcta desde aquí).
//     Queda documentado como PENDIENTE-COORDINACIÓN: agregar un
//     `insert into public.outbox (..., 'reservation.confirmed', ...)` dentro del bloque
//     `if (body.toStatus === "confirmada")` de `routes/reservas.ts` (línea ~335 al
//     momento de escribir esto) es la única pieza que falta del lado de ese lote.
//   - `cfdi.emitted` -> aviso de CFDI disponible: mismo caso -- handler listo,
//     `routes/cfdi.ts` (fuera de alcance) no emite todavía ese evento a `outbox`.
