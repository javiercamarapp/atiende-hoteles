...
//
// PENDIENTE-COORDINACIÓN cerrada por el integrador (merge H12a→main): `server.ts`
// (lote A) ya llama `startEmailOutboxScheduler(engine.admin, ...)` junto a los demás
// planificadores y usa `resolveEmailPort()` de este mismo archivo para `AppDeps.emailPort`
// -- este archivo sigue siendo también ejecutable como PROCESO INDEPENDIENTE (útil como
// cron adicional de recuperación, o si algún día `apps/api` corre en varias réplicas y
// se prefiere un solo drenador dedicado):
//   node --experimental-transform-types apps/api/src/emailOutbox/runEmailOutboxWorker.ts
//     (mismo criterio que `scripts/run-night-audit-scheduler.ts`);
...
// H12a/H12b pendiente-coordinación cerrada por el integrador: exportada para que
// `apps/api/src/server.ts` (lote A) resuelva el MISMO `EmailPort` real (Resend/SMTP/
// Fake, según lo que haya configurado en `.env`) para `AppDeps.emailPort` -- así
// `routes/registro.ts`/`routes/correo.ts` (envío síncrono durante el request) usan el
// mismo proveedor que este worker (envío asíncrono vía outbox), sin duplicar el criterio
// de selección Resend > SMTP > Fake en dos archivos.
export function resolveEmailPort(admin: DbClient): EmailPort {
