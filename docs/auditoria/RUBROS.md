...
**Dónde:** `apps/api/` (ADR-004): handlers de reservas/disponibilidad/folio/pagos, `set_config` de claims RLS por request, constraint `UNIQUE (tenant_id, idempotency_key)`, `pg_advisory_xact_lock` sobre `(hotel_id, room_type_id, fecha)`, tabla `outbox` con backoff; handlers que orquestan `EnergyPort`/`LockPort` (ADR-011, `packages/mcp-servers/energy/`, `packages/mcp-servers/locks/`) — lectura de telemetría, cambio de estado de HVAC, emisión/revocación de llave digital.

**Requisitos-ancla (REQ-*):** REQ-TEN-001, REQ-REC-003, REQ-REC-004, REQ-RES-002, REQ-RES-007, REQ-BO-027, REQ-BO-028, REQ-INT-007.

**Qué cuenta:** un `if` que detecta doble-reserva y no hace `return`; un handler de cargo a folio cuyo `INSERT` no está protegido por el `idempotency_key` y que un reintento de red duplica; un advisory lock que se pide para disponibilidad pero no para el cierre de folio (dos cajeros cierran el mismo folio a la vez); un `catch` que traga el error del conector PMS sin registrar qué reserva falló; un endpoint que acepta un `hotel_id` del body en vez de tomarlo del JWT de sesión (abre la puerta a operar sobre el hotel equivocado); un contrato que acepta una tarifa negativa o un `room_type_id` inexistente; un endpoint de `EnergyPort` (`setHvacState`) que acepta `roomId`/`circuitId` del body en vez de resolverlo por sesión, o que no aplica la guarda física 20-27°C (`REQ-BO-028/029`) antes de reenviar el comando al edge; un endpoint de `LockPort.issueKey` alcanzable sin verificar primero el evento del PMS (check-in pagado + identidad verificada, `REQ-REC-009`).
...
**Requisitos-ancla (REQ-*):** REQ-OBS-001, REQ-OBS-002, REQ-AGT-006, REQ-QA-005, REQ-BO-027.

**Qué cuenta:** un log de fallo que no dice **cuál** reserva o folio falló; una alerta ausente en el camino del dinero (cobro, cambio de tarifa); un error del conector PMS que se traga y devuelve 200 a WhatsApp; una variable de entorno que falta (p. ej. `ANTHROPIC_API_KEY`) y el sistema arranca igual simulando una respuesta de agente en vez de entrar en el "modo sin credenciales honesto" que ADR-006 exige; un `setup` que no deja el proyecto corriendo en una máquina limpia sin Docker (contradice ADR-003); PII en una traza persistida sin redactar (GOB-035); ausencia del laboratorio edge en CI (`REQ-QA-005`, ADR-011: contenedor con hub local simulado + simuladores MQTT) que debería confirmar en cada push que ninguna regla de energía puede accionar una cerradura y que los límites físicos de temperatura/humedad se respetan; un reporte de ahorro de energía (`REQ-BO-030`) o de llave digital que no declara explícitamente "pendiente de hardware/credenciales" cuando solo corrió contra `SimulatedEnergyAdapter`/`SimulatedLockAdapter`.
...
**Requisitos-ancla (REQ-*):** REQ-TEN-001, REQ-REC-003, REQ-REC-004, REQ-GOB-010, REQ-GOB-011, REQ-BO-027, REQ-INT-007, REQ-INT-008.

**Qué cuenta:** un dominio sin `CHECK` que acepta una tarifa negativa o un estado de reserva inventado (algo fuera de `cotizada|confirmada|check_in|en_estancia|check_out|cerrada|cancelada|no_show`); falta de `UNIQUE` donde la lógica asume unicidad — **overbooking** es exactamente este hallazgo: dos reservas confirmadas para la misma habitación la misma noche porque la base no lo impide, solo la aplicación cree evitarlo; falta de constraint de idempotencia `(tenant_id, idempotency_key)` que permite **doble cargo en el mismo folio**; un tipo de TypeScript más estricto que la columna real (la forma más común de mentirse); dinero tipado `float` en vez de `numeric(12,2)` (GOB-013); una transición de estado de `reservation` implementada como `UPDATE` destructivo en vez de evento append-only (ADR-005 exige lo segundo); RLS que se apoya en que la aplicación se porte bien en vez de en la política misma — la prueba real es si un script, una consola directa a Postgres o un bug futuro que se salte `apps/api/` puede leer o escribir una fila de la `org B` estando autenticado como la `org A` (aislamiento de tenant), o una fila de `hotel B` estando autenticado con rol solo en `hotel A` de la misma `org` (scope de hotel); una tabla de telemetría de energía (`readMeter`: `circuitId`, `kWh`, `kW`, `powerFactor`, `ts`, ADR-011) sin índice/`UNIQUE` por `(tenant_id, hotel_id, circuitId, ts)` que permita duplicar o cruzar lecturas entre hoteles; una tabla de llaves digitales sin constraint/trigger que impida un `issueKey` sin un evento previo de check-in pagado + identidad verificada (la aplicación cree evitarlo, la base no lo impide — mismo patrón que overbooking); RLS por tenant ausente o incompleta en las tablas de energía/cerraduras (mismo criterio que el resto del esquema, no una excepción por ser dominio nuevo).
