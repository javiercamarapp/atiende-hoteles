...
| REQ-AB-004 | hecho | Hecho (2026-09-10, posterior a este pase) | Reclasificado: el módulo de pedidos F&B (`apps/api/src/routes/pedidosFnb.ts`, `packages/domain-hotel/src/fnbAllergyGuard.ts`) sí existe y la guarda de alergias está conectada -- `tests/adversarial/alergias-confirmacion.spec.ts` (verde) exige confirmación humana del cocinero antes de asegurar que el platillo es seguro. Ver `docs/logs/REQ-AB-004/`. Nota: conteo agregado del documento (§0/§1) no recalculado en este pase, mismo tratamiento que REQ-HUE-014/REQ-HUE-021. |
...
| REQ-GOB-006 | hecho | Hecho (2026-09-10, posterior a este pase) | Reclasificado tras verificación independiente: la nota original de este pase describía una brecha en GOB-007 (el backlog de archivos aún no tiene un runner de I/O sobre disco), no el criterio LITERAL de `ACEPTACION.md` para GOB-006, que es de dominio puro (unit, "Depende de credenciales: No") y ya estaba implementado desde el commit `2e15f3c` (2026-09-08) sin que se hubiera actualizado esta fila ni la de REQUISITOS.md. `recordFailedAttempt()` (`packages/agent-core/src/backlog/backlogStateMachine.ts`) aplica la regla sobre el objeto `BacklogTask` de la máquina de estados de REQ-GOB-007: 3er fallo sin categoría reservada → `blocked` con diagnóstico; 3er fallo con `founderReservedCategory` dentro del catálogo cerrado de REQ-GOB-012 (24 categorías, migración 0081) → `needs-human` directo, nunca `blocked`. Verificado con 30/30 en `tests/unit/gob/tres-intentos-bloqueo.spec.ts` (incluye paridad campo por campo contra el enum real de Postgres vía PGlite), sin regresión en `tests/unit/gob/backlog-state-machine.spec.ts` (37/37) ni en el resto de la suite unitaria (1323/1324, 1 skip preexistente), `npm run typecheck` limpio y `npm run check:migraciones` en verde. `docs/logs/REQ-GOB-006/`. Nota: conteo agregado del documento (§0/§1) no recalculado en este pase, mismo tratamiento que REQ-AGT-004/REQ-SEG-012/REQ-OBS-010/REQ-HUE-021/REQ-HUE-014/REQ-SEG-007/REQ-OBS-007/REQ-SEG-013. |
