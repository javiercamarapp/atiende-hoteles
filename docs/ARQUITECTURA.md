...
## ADR-003 — Persistencia: Postgres real con migraciones SQL versionadas, PGlite para RLS rápido, `embedded-postgres` para concurrencia, esquema compatible con Supabase

**Contexto.** `docs/referencia/07-stack-viabilidad.md` es un experimento real ejecutado en esta máquina: PGlite 0.5.8 pasó 5/5 pruebas de RLS/triggers/`ON CONFLICT`/advisory lock, pero **serializa toda concurrencia** (1344 ms medidos donde debería haber ~300 ms si hubiera paralelismo real — Experimento 1). `embedded-postgres` 18.4.0-beta.17 arrancó un Postgres real (bajo Rosetta 2 en esta Mac) y demostró **concurrencia real** entre dos conexiones (302 ms medidos, Experimento 2). `npx supabase` (2.116.0) solo sirve para *scaffolding* de archivos (`init`, `migration new`, `functions new`) sin Docker; `db diff`, `gen types`, `start` fallan explícitamente pidiendo Docker incluso apuntando a un Postgres externo real (Experimento 3). H20 fija Postgres/Supabase multi-tenant con RLS `org → location` como modelo de datos (`docs/referencia/03-investigacion-H12-H21.md` §3.3).

**Opciones.**
1. Exigir al usuario instalar Docker/Supabase CLI antes de continuar.
2. Usar solo PGlite para todo (unitario e integración).
3. Postgres real con migraciones `.sql` versionadas a mano (usando `supabase migration new` únicamente como generador de nombre/plantilla, que sí funciona sin Docker), aplicadas por un runner propio contra dos motores según el tipo de prueba: **PGlite** para pruebas rápidas de RLS/lógica (vitest, sin proceso de SO) y **`embedded-postgres`** para integración/concurrencia real (advisory locks, idempotencia bajo contención), manteniendo el esquema compatible con Supabase (mismo patrón de claims que consumen las políticas RLS: `current_setting('request.jwt.claim.sub', true)`, roles vía `set local role authenticated`) para poder apuntar a un proyecto Supabase remoto más adelante sin reescribir RLS.

**Decisión.** Opción 3, exactamente como recomienda `07-stack-viabilidad.md` en su sección final "Recomendación de stack".

**Evidencia.** `docs/referencia/07-stack-viabilidad.md` Experimentos 1-3 y "Tabla comparativa (resumen de la decisión)"; `docs/referencia/03-investigacion-H12-H21.md` §3.3 (RLS `org→location`, entidades canónicas con JSON Schema).

**Consecuencias.** Ninguna prueba de contención/carrera tiene valor probatorio si corre solo en PGlite (riesgo 1 de 07-stack-viabilidad.md); toda prueba de overbooking/doble-cobro debe correr contra `embedded-postgres`. Las extensiones Postgres (`pgcrypto`, `pgjwt`, `pgsodium`, `pg_net`, `vector`) deben verificarse una por una antes de asumir portabilidad entre PGlite y `embedded-postgres` (riesgo 2). PostgREST/GoTrue (el contrato HTTP exacto de Supabase) **no** se reproduce en esta máquina — queda como integración pendiente explícita hasta tener Supabase remoto o Docker.

**Prueba que lo verifica.** Suite vitest+PGlite con aislamiento negativo de tenant (tenant A no ve/escribe filas de tenant B) corriendo en <2 s; suite vitest+`embedded-postgres` con dos clientes `pg.Client` concurrentes disputando el mismo `pg_advisory_xact_lock` sobre una reserva (equivalente a `order_idempotency_concurrency.sh` de Restaurantes); mismo archivo `.sql` de migración aplicado y verde contra ambos motores.
...
**Tenant y organización.** `tenant = hotel`, con un nivel superior opcional `org` (grupo hotelero) que agrupa varios `hotel` — igual que H20 exige (`org → location`, `location.kind='hotel'`, `docs/referencia/03-investigacion-H12-H21.md` §3.3). Un usuario tiene rol por `hotel` (tabla `hotel_staff`, mismo patrón que el experimento de 07-stack-viabilidad, `is_hotel_staff` security-definer calco de `is_restaurant_staff`).
...
**RLS por sesión.** Cada request abre una transacción que ejecuta `set local role authenticated` + `select set_config('request.jwt.claim.sub', <user_id>, true)` + `set_config('app.tenant_id', <hotel_id>, true)` antes de correr la query de negocio — mismo patrón verificado en el experimento de PGlite (07-stack-viabilidad.md Experimento 1) y en Restaurantes.
...
**Evidencia.** `docs/referencia/06-backoffice-agentes-likida.md` §2.2, §2.3, §2.5, §2.6, §2.7; `docs/referencia/04-gobierno-y-protocolo.md` GOB-026, GOB-032, GOB-036, GOB-037; `docs/referencia/01-blueprint-y-decision-llm.md` §3 puntos 1-3; `docs/referencia/03-investigacion-H12-H21.md` BP-043/054/070 (vía tabla de requisitos H12-H21, sección 2.5-2.11 del documento fuente `03-investigacion-H12-H21.md`, en realidad citada desde `01-blueprint-y-decision-llm.md` BP-043/BP-054/BP-070 — mismo repositorio de referencia).
...
- **Adversariales de aislamiento/permisos**: batería que intenta cruzar tenant (hotel A leyendo hotel B), escalar rol (huésped intentando ver back office), reenviar webhook con firma inválida, doble-cobrar con el mismo `idempotency_key` — mismo espíritu que la skill `auditoria-semanal` de Likida (06 §5.3: "un ataque que rompe = bug encontrado con su prueba ya lista").
...
| **H7** | Runtime de agentes por rol (Sonnet/Haiku/Opus vía env) + `audit_log` encadenado + `roi_event` | GOB-014, GOB-032, GOB-036, GOB-037, H17-001 |
...
| 3 | Supabase multi-tenant (Postgres+RLS+GoTrue+PostgREST) como plataforma | Postgres propio (PGlite para unit, `embedded-postgres` para integración) + JWT propio con el mismo contrato de claims RLS | Docker/Supabase CLI no funcionan localmente para `start`/`db diff`/`gen types` (verificado, `07-stack-viabilidad.md` Experimento 3); esquema queda compatible para migrar a Supabase remoto |
...
Stack decidido: monorepo **npm workspaces + Turborepo**; frontend **Vite+React+TS+shadcn+Tailwind** portando identidad/tokens/primitivos de `atiende-restaurantes` con navegación hotelera y mobile real (bottom-nav) que Restaurantes no tiene; backend **Hono + JWT propio (`jose`)** con RLS por sesión (`set_config` de claims), tenant=hotel, matriz de 9 roles, idempotencia y `pg_advisory_xact_lock`; persistencia **Postgres real versionado por SQL**: **PGlite** para RLS/unitarias rápidas y **`embedded-postgres` 18.4** (Postgres real, concurrencia verificada 302 ms) para integración/contención; agentes con patrón Likida (tools sin datos del modelo, loop-guard, presupuesto, `needs_approval` obligatorio) y runtime Sonnet/Haiku/Opus por rol vía variable de entorno, con modo "sin credenciales" honesto; integraciones PMS/WhatsApp/pagos/CFDI/voz como puerto+adaptador+contrato, **pendientes de credenciales** sin marcarse completas. Tres desvíos principales respecto a H20: (1) npm workspaces en vez de pnpm por B-002; (2) Postgres propio (PGlite+`embedded-postgres`) en vez de Supabase gestionado, porque Docker/Supabase CLI no funcionan en esta máquina; (3) cola de aprobación propia en vez de Inngest, por no haberse verificado su ejecución sin credenciales en este entorno.
