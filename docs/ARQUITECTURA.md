...
**`hotel-staff-pwa` (REQ-TEN-003/REQ-UX-003): qué es y qué no es, decidido explícitamente.** El nombre viene literal de la fuente (BP-109) pero no estaba decidido si implica propiedades reales de PWA o es solo la vista responsiva de housekeeping/mantenimiento descrita arriba. Decisión: **sí es una PWA mínima real**, no solo una vista responsiva — se añade `manifest.json` (nombre, iconos, `display: standalone`, `start_url` a la vista de tareas del turno) y un service worker que cachea el shell de la app (JS/CSS/iconos) para que la app cargue e instale sin conexión intermitente en zonas del hotel con wifi débil; **no** incluye sincronización de datos offline-first (crear/editar tareas sin conexión) — esa capacidad es la del "modo huracán/degradado" (`REQ-REC-013`), un requisito distinto con su propio mecanismo (ver ADR-011/BLOQUEOS D-003), y queda fuera del alcance de esta PWA. `hotel-staff-pwa` es instalable (ícono en pantalla de inicio) y muestra el shell cacheado con datos honestamente "sin conexión" (no simulados) cuando no hay red, reutilizando `EstadoError`/`EstadoVacio`.
...
**Prueba que lo verifica.** Captura Playwright/Chrome del sistema en viewport `375×812` (mobile) y `1440×900` (desktop) del sidebar hotelero y del panel de housekeeping, comparada visualmente contra las capturas equivalentes de `atiende-restaurantes` (mismo Chrome, mismos flags `--force-prefers-reduced-motion`); suite `axe-core` sin violaciones críticas en las rutas principales; prueba de `ThemeSelector` (`role`/`aria-checked`) portada 1:1; auditoría Lighthouse/Playwright confirma `manifest.json` válido y service worker registrado (instalable) en `hotel-staff-pwa`, sin asumir sincronización de datos offline (eso es `REQ-REC-013`, fuera de esta PWA).
...
## ADR-012 — REQ-GOB-005 ("cierre de tarea"): tres herramientas separadas, no una sola que verifique evidencia+commit+criterio

**Contexto.** `REQ-GOB-005` exige, en un solo criterio de aceptación, cuatro cosas distintas sobre el cierre de una tarea: (a) verificación en verde con evidencia guardada, (b) un commit de cierre siguiendo Conventional Commits ("uno por tarea"), (c) el estado pasa a `review`, y (d) toda decisión de diseño no trivial genera un ADR. Al construir este ADR, (a) ya tenía dueño real y probado (`REQ-OBS-010`, `scripts/checks/evidencia-obligatoria-cierre.ts`) y la puerta previa a la implementación sin criterio verificable ya tenía el suyo (`REQ-GOB-003`, `beginImplementation` en `backlogStateMachine.ts`) — ambos construidos en rondas anteriores de este mismo repo, siguiendo el patrón autocontenido de script sobre `tasks/**` que `gate-por-tarea.ts`/`scope-de-tarea.ts` ya establecieron. El encargo de REQ-GOB-005 señaló explícitamente este solape y dejó la decisión de diseño al agente que lo tomara: ¿una sola herramienta de "cierre de tarea" que verifique las tres cosas (evidencia + commit + criterio), o herramientas separadas?

**Decisión.** Se mantienen **tres scripts separados**, uno por invariante, en vez de fusionarlos en un verificador único de "cierre de tarea":

- `scripts/checks/evidencia-obligatoria-cierre.ts` (REQ-OBS-010) — evidencia guardada.
- `packages/agent-core/src/backlog/backlogStateMachine.ts::beginImplementation` (REQ-GOB-003) — criterio de aceptación verificable, puerta de dominio previa a `doing`, no una revisión estática de texto libre.
- `scripts/checks/cierre-tarea-conventional-commits.ts` (este ADR, REQ-GOB-005) — commit de cierre único en formato Conventional Commits.

La porción "toda decisión de diseño no trivial genera un ADR" (d) queda como disciplina documentada, no como revisión automática: un análisis estático de texto no puede distinguir de forma confiable una decisión "trivial" de una que no lo es sin arriesgar falsos positivos/negativos masivos (este propio ADR-012 es evidencia de que la disciplina se sigue en la práctica, no solo se declara). "El estado pasa a `review`" (c) es una transición que ya vive en `backlogStateMachine.ts` (la máquina de estados que GOB-001/GOB-007 ya modelan); no se le suma un cuarto check que solo repita esa transición.

Razones para NO fusionar (a)+(b)+(c) en un solo verificador:

1. **Consistencia con el patrón ya establecido del repo.** Cada invariante de gobierno sobre `tasks/**` ya vive en su propio script autocontenido, sin dependencias cruzadas: `gate-por-tarea.ts` (REQ-AGT-019), `scope-de-tarea.ts` (REQ-GOB-004), `evidencia-obligatoria-cierre.ts` (REQ-OBS-010), `no-comandos-destructivos-agente.ts` (REQ-GOB-009), `pms-mirror-solo-lectura.ts` (REQ-GOB-013). Fusionar rompería ese patrón para un solo requisito sin ninguna ganancia técnica que lo justifique.
2. **Responsabilidad única y blast radius acotado.** Un fallo de "falta evidencia" y un fallo de "commit sin Conventional Commits" son causas raíz distintas con dueños de corrección distintos (el primero se corrige guardando logs; el segundo, reescribiendo un mensaje de commit). Un solo script que reporte ambos mezclados obliga a leer y descartar ruido para llegar a la causa real; dos pasos de CI independientes (mismo patrón de "falla rápido" de ADR-009) señalan la causa exacta de inmediato.
3. **Cobertura de pruebas ya construida y verde no se toca.** `evidencia-obligatoria-cierre.spec.ts` (24 pruebas) y `bloqueo-sin-criterio.spec.ts` (9 pruebas) ya existen, ya pasan, y ya fueron verificados end-to-end contra el repo real en rondas anteriores. Fusionarlos en un módulo nuevo obligaría a reescribir esa cobertura o a importar código de un frente ajeno (arriesgando su cobertura ya probada) — exactamente lo que el patrón autocontenido de estos checks busca evitar (ver cabecera de cualquiera de ellos: "NO depende de ese módulo... para no arriesgar su cobertura ya probada").
4. **Costo de mantenimiento menor.** Tres scripts de ~150-200 líneas cada uno, cada uno con una sola invariante y una sola razón para cambiar, son más fáciles de razonar y modificar sin efectos colaterales que un verificador combinado con múltiples modos de fallo entrelazados.

**Consecuencias.** `.github/workflows/ci.yml` gana un paso más de fallo rápido (`cierre-tarea-conventional-commits.ts`, junto a `evidencia-obligatoria-cierre.ts`/`gate-por-tarea.ts`) en vez de que uno de los pasos existentes crezca en alcance; el precedente queda escrito aquí para que un futuro quinto/sexto invariante de "cierre de tarea" (si aparece) seguirá el mismo patrón de script independiente salvo que una razón técnica concreta (no solo preferencia) justifique fusionar.

**Prueba que lo verifica.** Los tres scripts corren de forma independiente en CI y en `npm test`, cada uno con su propia suite de pruebas (`tests/unit/gob/evidencia-obligatoria-cierre.spec.ts`, `tests/unit/gob/bloqueo-sin-criterio.spec.ts`, `tests/unit/gob/cierre-tarea-conventional-commits.spec.ts`), sin importar código entre sí (verificado por revisión de los `import` de cada archivo); `docs/logs/REQ-GOB-005/` documenta la verificación end-to-end de `cierre-tarea-conventional-commits.ts` contra una tarea de muestra real ya cerrada de este propio repo (REQ-GOB-004).

---

## Resolución de las 10 contradicciones de `docs/REQUISITOS.md` §4

`docs/REQUISITOS.md:401-414` (§4) delega explícitamente a este documento la decisión de cada una de las 10 contradicciones "antes de construir el módulo afectado". Se resuelven aquí, una por una y por nombre; las que tocan el catálogo de decisiones reservadas al fundador (`REQ-GOB-012`, GOB-051/052) se exponen en `docs/BLOQUEOS.md` sin bloquear el trabajo de construcción del resto del módulo.

1. **PMS propio vs. "no construir PMS en fase 1".** Resuelto: "fase 1" es H1-H9 de la tabla de hitos de este documento — ningún hito hasta H9 construye `REQ-REV-012` (PMS ligero propio). `REQ-REV-012` es P2 y explícitamente posterior a validar el modelo con conectores externos (Cloudbeds primero, `REQ-INT-001`); no se agenda hito propio para él en esta ronda.
2. **Supabase (H20) vs. modo huracán/degradado offline (`REQ-REC-013`).** Resuelto el mecanismo técnico, expuesta la política al fundador: la copia local cifrada de `REQ-REC-013` se reconcilia con Supabase usando el mismo patrón de `RawEvent` inmutable + command bus idempotente + Outbox que ADR-004/ADR-005 ya definen para conectores externos — el edge local encola eventos localmente durante el corte y los reproduce contra el mismo `Adapter.normalize`/Reducer al reconectar, sin mecanismo nuevo. Lo que **no** se decide aquí (reservado al fundador, GOB-052 "protocolos de huracán: umbrales de aviso, mensajes masivos") son los umbrales de activación y el contenido de los mensajes masivos — ver `docs/BLOQUEOS.md` D-002.
3. **Tipo de cambio de referencia distinto (H20: 18.5 `[E]` vs. H17: 17.0 `[DATO]` MXN/USD).** Resuelto: ninguno de los dos es un valor de producción; `REQ-RES-015` (multi-moneda) ya exige que el sistema use un tipo de cambio vigente configurable en tiempo de ejecución, no una constante de documento. Los dos valores citados son solo supuestos de presupuesto de infraestructura (H20) y de modelo de ROI (H17) respectivamente, de fechas de investigación distintas; no requieren reconciliarse para construir, porque ningún código de producción los usa como fuente de verdad.
4. **Cifra de ROI de la suite sin reconciliar (H17: MXN 945k/355k conservador vs. blueprint 10-palancas: MXN 450-740k).** Resuelto a favor del catálogo H17: `REQ-REV-018`/`REQ-REV-019` ya exigen que todo `ROIEvent` lleve `monto_verificado`/`monto_estimado`/`método_contrafactual`/`confianza` por agente — ese es el formato del catálogo de 16 agentes de H17, no el de las "10 palancas" del blueprint (que es una cifra de mercadeo sin ese desglose auditable). El techo de precio de `REQ-REV-019` (≤20-30% del valor conservador) se calibra contra la cifra conservadora de H17 (MXN 355k), no contra el rango del blueprint.
5. **Runtime de agentes distinto entre líneas de negocio (Claude API para Hoteles/Restaurantes vs. OpenAI Agents SDK para Licitaciones, GOB-031).** Resuelto: fuera de alcance de este ADR. Licitaciones es una línea de negocio distinta con su propio repositorio/reglas (GOB-031, excluido explícitamente de `docs/REQUISITOS.md` §2); Atiende Hoteles usa exclusivamente el `agent-runtime` de Claude API (ADR-006). No se comparte infraestructura de evals/observabilidad entre ambos runtimes en esta fase; si en el futuro se comparte Langfuse/evals entre líneas, es una decisión posterior no bloqueante hoy.
6. **Decisión LLM declarada "no final" (Opción C).** Resuelto para efectos de construcción, expuesta la confirmación final al fundador: ADR-006 opera con la Opción C (Sonnet 5/Haiku 4.5/Opus 5) como default de producción, exactamente como el propio documento fuente indica mientras no haya decisión distinta del fundador. Cambiar de opción (o confirmarla como definitiva) es la decisión reservada que `REQ-AGT-011`/`REQ-GOB-012` ya cubren — ver `docs/BLOQUEOS.md` D-003 para el registro explícito de que sigue sin confirmación final.
7. **Comisión "no permitida" (`REQ-GOB-017`) vs. comisión de agentes externos 2-4% (H06-015).** Resuelto: no es la misma categoría. `REQ-GOB-017` prohíbe cobrar comisión **al huésped o a la reserva en general** (para no calificar como "plataforma digital" bajo Retur-Q); el 2-4% de H06-015 es un **success-fee cobrado al hotel** sobre reservas directas específicamente atribuidas a un agente de IA externo (MCP/rail agéntico) — mismo mecanismo de atribución por canal que ya exige `REQ-RES-020`. Ambos conviven: `REQ-GOB-017` ya lo redacta así ("SaaS fijo más un porcentaje acotado de reservas directas atribuidas al agente").
8. **Orden de prioridad de PMS inconsistente (BP-117 §12.1: Mews P1 sin mencionar Cloudbeds vs. consenso mayoritario H01-H04/H08/H09/H11/H15-001: Cloudbeds primero).** Ya resuelto en la práctica y confirmado aquí explícitamente: `REQ-REV-010`/`REQ-INT-001`/ADR-007 adoptan Cloudbeds como primer conector (orden Cloudbeds → Mews → SiteMinder/pmsXchange → OHIP). La sección §12.1 del blueprint queda como desviación de fuente única no seguida, documentada aquí para que quede explícito que no se siguió.
9. **Cerraduras: "hotel sin recepción nocturna" (H07-039/BP-094) vs. prohibición de automatizar llaves (GOB-052/BP-066/BP-092).** No se decide aquí — es exactamente la categoría que GOB-052 reserva al fundador ("modo hotel sin recepción nocturna" y "control físico de AC/cerraduras/emisión de llaves" son dos ítems separados de ese catálogo). Lo que sí se fija en ADR-011: el `LockPort` nunca emite/revoca una llave sin autenticación fuerte por canal autenticado + evento del PMS (check-in pagado + identidad verificada), sin importar si el hotel tiene o no recepción nocturna — esa garantía es independiente de la decisión pendiente. El módulo "sin recepción nocturna" en sí (`REQ-GOB-018`, fase posterior) no se construye hasta que el fundador apruebe explícitamente la integración de cerraduras — ver `docs/BLOQUEOS.md` D-004.
10. **OCR/VLM autoalojado de origen chino (LLM-009) vs. postura occidental de la Opción C (LLM-017/021).** Ya resuelto por el propio catálogo de gobierno, confirmado aquí: LLM-023 exige confirmación separada del fundador antes de desplegar PaddleOCR-VL/GLM-OCR/Qwen3-Embedding en producción, y esa confirmación es distinta de (y no incluida en) la aprobación de la Opción C. `REQ-AGT-013`/`REQ-AGT-014` (OCR/embeddings autoalojados) permanecen `pendiente` (reclasificado `pendiente-decisión` el 2026-09-08, ver `docs/BLOQUEOS.md` D-006) hasta esa confirmación explícita; no se activa ningún servicio lateral de origen chino en código de producción sin ella. El guardrail estático que sí es verificable sin esa decisión (0 llamadas a un LLM de canal o a un endpoint chino de primera parte para OCR en todo el árbol) está construido y en verde: `scripts/checks/ocr-aislado-sin-internet.ts` + `tests/unit/agent-core/ocr-aislado-sin-internet.spec.ts`.
...
| **H2** | Backend Hono + JWT propio + matriz de roles + `set_config` de claims + idempotencia + advisory locks + outbox | GOB-013, GOB-042, H15-007, H15-020 — **REQ-TEN-003, REQ-REC-003, REQ-REC-004, REQ-INT-012, REQ-INT-014, REQ-GOB-007, REQ-GOB-009** |
