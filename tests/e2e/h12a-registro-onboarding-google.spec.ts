...
// Merge H12a→main (integrador): las 7 capturas de abajo usaban una ruta relativa
// ("tests/e2e/screenshots/...") que Playwright resuelve contra su cwd real
// (apps/web/, donde vive playwright.config.ts) -- terminaban en
// apps/web/tests/e2e/screenshots/, un directorio nuevo y sin trackear, en vez de
// tests/e2e/screenshots/ (raíz del repo) donde ya viven las capturas de H4/H5/H6/H7.
// Mismo patrón de `SCREENSHOTS_DIR` que h6-housekeeping-mantenimiento-mensajeria.spec.ts.
...
test.describe.serial("H12a · registro → verificación → onboarding → Google", () => {
  ...
  test("1. registro: formulario → revisa tu correo", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "h12a-registro-formulario.png"), fullPage: true });
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "h12a-registro-revisa-correo.png"), fullPage: true });
  });
  ...
  test("2. verificación de correo (token leído de la base, simulando el correo capturado por el proveedor fake)", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "h12a-verificar-correo.png"), fullPage: true });
  });
  ...
  test("3. onboarding: login → tipo de habitación → zona horaria → invitar equipo", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "h12a-onboarding-habitaciones.png"), fullPage: true });
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "h12a-onboarding-equipo.png"), fullPage: true });
  });
  ...
  test("4. login con Google (servidor falso): recorre TODA la cadena de redirecciones hasta /resumen", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "h12a-google-login-resumen.png"), fullPage: true });
  });
  ...
  test("5. login con Google, correo no verificado: termina en /login con el error humano", async ({ page }) => {
    ...
    await page.screenshot({ path: path.join(SCREENSHOTS_DIR, "h12a-google-error.png"), fullPage: true });
  });
});
