  ...
  try {
    apiProc = spawn("node", ["--experimental-transform-types", "src/server.ts"], {
      ...
      env: {
        ...
        NODE_ENV: "test",
        // H8 (auditoria-1/seguridad.md [MEDIO] CORS): la lista blanca ya no cae a un
        // comodín ni al puerto fijo de Vite dev -- el `apps/web` de esta prueba corre
        // en un puerto dinámico, así que hay que declararlo explícitamente (mismo
        // patrón que login-real-y-resumen.spec.ts/h4-reserva-real-desde-ui.spec.ts).
